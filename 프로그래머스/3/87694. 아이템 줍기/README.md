...
### 풀이
사각형 부분을 모두 순회하여 -1로 채워주고, 현재 위치로부터 갈 수 있는 다음 위치를 큐에 넣어주어 최단 경로를 구한다.<br>
시작 위치는 캐릭터 위치. 배열값이 -1이 아니면 이미 방문한 경로. <br>
만약 사각형 테두리가 아닌 내부에 있다면 (테두리보다 안쪽에 있는지 사각형 순회하여 확인) 방문할 수 없는 경로이다. <br>
방문할 수 있는 곳을 큐에 넣고, 해당 위치의 배열값을 갱신해줌.  

#### 아이디어
좌표를 모두 2배 해준다. 테두리가 인접하는 경우가 발생할 수 있는데, 이 경우에 최단경로가 잘못 구해진다.<br>
그렇기 때문에 좌표를 2배 해주어 정밀도를 높인다. <br>
리턴할 때는 까먹지 말고 /2 한 값을 리턴하자.
